...
const char *argp_program_version = "scom 0.2";
...
//	>pathtofile		Set file for logging data we recv (just > turns off logging)
//	<pathtofile		Set file for echoing data we send (just < turns off echoing)
int waitForCommand(char **buffer) {
  int cmd = 0;
...
void sendBytes(char *data, int size) {
  int i=0;
...
void recvHexString() {
  ...
  if(count) {
	...
	if((buffer[i] < ' ') || (buffer[i] > '~')) {
      ...
      fprintf(stdout, "... (%d)  [%.*s]...", count, 4, buffer);
    }
...
void recvEvent() {
  ...
  if(count >= 8) {
	...
	case 0xFE:
	    ...
	    break;
	default:
	    //fprintf(stdout, "OTHER BYTE: %02X\n", sync_byte);
	    return;
    }

    read(outpfd, &event, sizeof(event));

    if(recvfd != -1) {
      write(recvfd, &event, sizeof(event));
    }
